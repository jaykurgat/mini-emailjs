...
/**
 * Build a clean HTML auto-reply email body.
 * The plain-text message (with \n newlines) is converted to HTML paragraphs.
 *
 * @param {string} message - the rendered auto-reply body text
 * @param {string} projectName - used in the footer
 */
export function buildAutoReplyBody(message, projectName = '') {
  const paragraphs = message
    .split('\n')
    .map(line => line.trim())
    .reduce((acc, line) => {
      if (line === '') {
        acc.push(null); // paragraph break
      } else {
  ...
  const bodyHtml = paragraphs
    .map(p => `<p style="margin:0 0 14px;color:#334155;font-family:sans-serif;font-size:14px;line-height:1.6;">${escapeHtml(p)}</p>`)
    .join('\n');

  return `
  <div style="font-family:sans-serif;max-width:560px;margin:0 auto;">
    <div style="background:#080D18;padding:20px 28px;border-radius:12px 12px 0 0;">
      <h2 style="color:#fff;margin:0;font-size:16px;font-weight:600;">${escapeHtml(projectName)}</h2>
    </div>
    <div style="background:#F5F7FB;padding:28px;border-radius:0 0 12px 12px;border:1px solid #E2E8F0;border-top:none;">
      ${bodyHtml}
